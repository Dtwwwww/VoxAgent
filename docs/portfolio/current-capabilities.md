...
本文只描述当前仓库可运行、可测试或已有实测报告的能力。路线图中的安全工具、MCP、Hybrid RAG、可观测性和交付增强不计入当前能力。
...
| 流式对话 | Ollama 流式生成、文字/语音统一消息流、轮次和请求 ID 关联、取消与旧事件隔离 | `backend/src/voxagent/conversation/`、`frontend/src/useVoiceSession.ts` | 是有界的会话编排器，不是 ReAct / LangGraph Agent |
...
- 不应写“已实现 Agent 工具调用”：当前没有原生 Function Calling、工具 Schema、权限分级、确认卡、防重放或审计闭环。
