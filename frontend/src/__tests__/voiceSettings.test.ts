...
describe("voice settings", () => {
  ...
  it("stores only retained public settings and reconciles unknown voices to server default", () => {
    ...
    expect(JSON.parse(stored)).toEqual({ voiceKey: "default_voice", speed: 0.8 });
    ...
    expect(reconcileVoiceSettings({ voiceKey: "removed", speed: 1.2 }, voices)).toEqual({ voiceKey: "default_voice", speed: 1.2 });
  });
});
