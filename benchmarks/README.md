...
The validator exits nonzero on any mismatch. `voxagent probe-resources --help` documents the observation/soak harness; its schema records sampling configuration, raw samples, stop reason, and memory-pressure thresholds. Plan 02 will append measured ASR and TTS candidates without changing the existing LLM evidence.
