...
flowchart LR
    ...
    API --> ORCH[ConversationOrchestrator<br/>流式会话与上下文编排]
    ORCH --> LLM[Ollama / Qwen3 4B]
...
- 当前没有 Function Calling / ReAct 状态图、安全工具权限分级、MCP Server / Client；不能把项目描述成已完成 Agent 工具平台。
...
2. **JR-02 安全 Agent 工具：** 有界状态图、原生 Tool Calling、权限确认、防重放和审计。
